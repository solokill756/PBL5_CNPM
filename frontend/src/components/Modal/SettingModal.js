...
export default function SettingModal() {
  ...
  const setShowFrontFirst = useFlashcardStore(
    (state) => state.setShowFrontFirst
  );
  const showOnlyStarred = useFlashcardStore((state) => state.showOnlyStarred);
  ...
  const buttonLabel = showFrontFirst
  ...
  return (
    <Modal isOpen={isModalOpen} onClose={closeModal} size="max-w-3xl">
      <div className="my-4 !px-4 space-y-6">
        ...
        {/* Mặt trước hiển thị */}
        <div className="flex items-center w-full">
          <span className="text-base font-medium w-[82%]">
            Mặt trước hiển thị
          </span>
          <DropdownDefault
            className="flex-1"
        ...
        <button
          onClick={() => {
            useFlashcardStore.getState().setCurrentIndex(0);
            useFlashcardStore.getState().incrementRestartKey();
          }}
          className="text-red-700 hover:text-red-800 text-base font-semibold"
        >
