...
const FlashCardItem = ({id, title, avatar, number, author, loading = false }) => {
  return (
    <Link to={id ? `flashcard/${id}` : '#'} className="scrollable-item flex-shrink-0 overflow-hidden snap-start basis-1/2 px-2">
      <div className="active:border-solid active:border-2 active:border-red-300 relative p-6 space-y-10 bg-white border-2 cursor-pointer border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700 
        ...
        <span className={`px-2 py-1 text-xs font-semibold text-gray-800 ${loading ? 'bg-inherit' : 'bg-red-100'} rounded-full`}>
          {loading ? <Skeleton width={100} height={20} /> : `${number} thuật ngữ`}
        </span>
        ...
        <Link to={''} className="flex items-center w-fit">
          {loading ? (
            <>
              ...
              <span className="ml-2"><Skeleton width={80} /></span>
            </>
          ) : (
            <>
