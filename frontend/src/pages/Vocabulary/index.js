...
const Vocabulary = () => {
  ...
  const { selectedWord, error: vocabError, clearResults } = useVocabularyStore();
  const { 
    categories, 
    userLevel, 
    loadingStates,
  ...
  useEffect(() => {
    ...
    const initializeData = async () => {
      if (shouldRefresh()) {
        ...
        try {
          const forceRefresh = lastLocationKey.current !== location.key || 
                              categories.length === 0 || 
                              !userLevel.user_id;
  ...
  }, [location.key, axios, categories.length, userLevel.user_id, hasInitialized]);

  useEffect(() => {
    if (location.pathname === '/vocabulary' && !location.pathname.includes('/vocabulary/')) {
      clearResults();
    }
