...
function TestAgain() {
  ...
  // Lấy danh sách câu sai từ quiz store
  const { wrongQuestions } = useQuizStore();
  
  // Sử dụng câu sai thay vì displayDeck
  ...
  // Khởi tạo timer ngay khi component mount và có dữ liệu
  useEffect(() => {
    if (questionsToReview.length > 0 && !quizStarted) {
      const totalTime = questionsToReview.length * 25; // 25 giây mỗi câu
      setTimeLeft(totalTime);
      setQuizStarted(true);
      setIsTimeUp(false);
    }
  }, [questionsToReview.length, quizStarted]);
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      {/* Timer hiển thị cố định ở góc trên bên phải */}
      {quizStarted && (
