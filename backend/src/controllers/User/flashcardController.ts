...
import {
  ...
  getClassOfUser,
  getAllExplanation,
} from "../../services/User/flashcardService";
...
const rateListFlashcardController = async (req: Request, res: Response) => {
  ...
  try {
    const flashcards = await rateListFlashcard(
      list_id,
      Number(rate),
      userId,
      comment
...
const addListFlashcardToClassController = async (
  req: Request,
  res: Response
) => {
...
const shareLinkListFlashcardToUserController = async (
  req: Request,
  res: Response
) => {
