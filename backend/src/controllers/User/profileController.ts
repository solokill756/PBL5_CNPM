...
import profileService from "../../services/User/profilePageService";
  ...
  try {
    ...
    const updateProfile = {
      ...
      password: req.body.password
        ? await bcrypt.hash(req.body.password as string, saltRounds)
        : undefined,
...
const setReminder = async (req: Request, res: Response) => {
  try {
    ...
    const data = await profileService.setReminder(
      userId,
      reminderTime,
      reminderStatus
    );
...
const setReminderClass = async (req: Request, res: Response) => {
  try {
    ...
    try {
      const profile = await profileService.setReminderClass(
        userId,
        classId,
        reminderStatus
      );
