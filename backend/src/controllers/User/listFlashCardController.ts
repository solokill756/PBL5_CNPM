...
import listFlashCardService from "../../services/User/listFlashCardService.";
...
const getForgetFlashcard = async (req: Request, res: Response) => {
  const user_id = (req as any).user.user_id;
  ...
  try {
    const data = await listFlashCardService.getForgetFlashcard(
      Number(length),
      user_id
    );
    sendSuccess(res, data, "Forget flashcards retrieved successfully");
  } catch (error: any) {
    sendError(
      res,
      "Error retrieving forget flashcards",
      500,
      error?.message || error
    );
  }
