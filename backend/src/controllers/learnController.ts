...
import { addFlashcardToLearn, getFlashCardLearn, notRemenberFlashcard, remenberFlashcard, resetLearn, saveProcess } from "../services/learnService";
...
const remenberFlashcardController = async (req: Request, res: Response) => {
    try {
        ...
        const user_id =  (req as any).user.user_id;
        const flashcard = await remenberFlashcard(flashcard_id, user_id);
        sendSuccess(res, flashcard);
    } catch (error : any) {
        sendError(res, error.message, 500);
    }
...
const notRemenberFlashcardController = async (req: Request, res: Response) => {
    try {
        const { flashcard_id } = req.params;
        const user_id =  (req as any).user.user_id;
        const flashcard = await notRemenberFlashcard(flashcard_id, user_id);
        sendSuccess(res, flashcard);
    } catch (error : any) {
        sendError(res, error.message, 500);
    }
...
const saveProcessController = async (req: Request, res: Response) => {
    try {
        ...
        const user_id =  (req as any).user.user_id;
        const process = await saveProcess(list_id, user_id, Number(number_word_forget), flashcard_id);
...
export { addFlashcardToLearnController, remenberFlashcardController , notRemenberFlashcardController, saveProcessController, getFlashCardLearnController, resetLearnController }; 
