...
const remenberFlashcard = async (flashcard_id: string, user_id: string) => {
    try {
...
const notRemenberFlashcard = async (flashcard_id: string, user_id: string) => {
    try {
        const flashcardUser = await db.flashcardUser.findOne({
            where: {
                flashcard_id: flashcard_id,
...
const saveProcess = async (list_id: string, user_id: string, number_word_forget: number , flashcard_id: string) => {
    try {
...
const getFlashCardLearn = async (list_id: string, user_id: string) => {
    try {
        ...
        const flashcards = await db.flashcard.findAll({
            ...
            order: [
                [db.flashcardUser, 'last_review', 'ASC'],
            ],
        });
        ...
        if (listFlashcard.last_review_flashcard_id) {
            const index = shuffleFlashcards.findIndex(
                (flashcard: any) => flashcard.flashcard_id === listFlashcard.last_review_flashcard_id
            );
            if (index !== -1) {
...
const resetLearn = async (list_id: string, user_id: string) => {
    try {
        ...
        const flashCardIds = flashcards.map((flashcard: any) => flashcard.flashcard_id);
