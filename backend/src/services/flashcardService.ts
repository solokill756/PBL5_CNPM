...
const updateReviewCount = async (flashcard_id: string, user_id: string) => {
  try {
    ...
    const updatedFlashcard = await db.flashcardUser.update(
...
const updateLastReview = async (flashcard_id: string, user_id: string) => {
  const updatedFlashcards = await db.flashcardUser.update(
...
const getFlashcardByListId = async (list_id: string, user_id: string) => {
  try {
    ...
    const number_flashcard_learn = await db.flashcardUser.count({
      where: {
        flashcard_id: flashCardID.map(
...
const checkRateFlashcard = async (
  list_id: string,
  user_id: string
): Promise<{ rate: number } | boolean> => {
  try {
...
const shareLinkListFlashcardToUser = async (list_id: string, email: string) => {
  try {
    const listFlashcard = await db.listFlashcard.findOne({
      where: { list_id },
    });
...
const getAllExplanation = async (flashcard_id: string) => {
  try {
    ...
    if (flashcard.ai_explanation) {
      try {
        ...
        if (
          ...
          console.log(
            `Using cached AI explanation for flashcard: ${flashcard_id}`
          );
        ...
        console.warn(
          `Invalid JSON in ai_explanation for flashcard ${flashcard_id}, generating new explanation`
        );
    ...
    const response = await fetch(
      "http://itkotoba-al-server.azurewebsites.net/analyze",
      {
        method: "POST",
        headers: {
