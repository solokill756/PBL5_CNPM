...
const setReminder = async (userId: string, reminderTime: Date, reminderStatus: boolean): Promise<any> => {
    try {
        const user = await db.user.findByPk(userId);
        if (!user) {
            throw new Error("User not found");
        }
        await db.user.update({ reminder_time: reminderTime, reminder_status: reminderStatus }, { where: { user_id: userId } });
...
const getReminder = async (userId: string): Promise<any> => {
    try {
        ...
        return { reminder_time: user.reminder_time, reminder_status: user.reminder_status };
    } catch (error) {
        throw new Error("User not found");
    }
}
...
const setReminderClass = async (userId: string, classId: string, reminderStatus: boolean): Promise<any> => {
    try {

        await db.classMember.update({ reminder_status: reminderStatus }, { where: { class_id: classId, user_id: userId } });
...
const updateProfilePicture = async (userId: string, profilePicture: string): Promise<any> => {
    try {
        await db.user.update({ profile_picture: profilePicture }, { where: { user_id: userId } });
...
export default { getProfile, updateProfile, setReminder, getReminder, getReminderClass, setReminderClass, deleteUser, updateProfilePicture };
